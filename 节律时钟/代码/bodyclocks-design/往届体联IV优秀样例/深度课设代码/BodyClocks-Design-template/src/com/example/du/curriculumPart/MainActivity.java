...
public class MainActivity extends Activity implements OnClickListener {
	...
	private final static String[] ORDER_READY = { "小组摘要", "组长未点名", "周志员未点名",
			"需求员未点名", "优裁员未点名", "展示员未点名" ,"全组点名未完成"};
	private final static String[] ORDER_OK = { "体联IV15#", "组长黄姗姗", "周志员唐萃希",
			"需求员张政", "优裁员张铭", "展示员郭会朋","全组点名已完成" };
	...
	private void show_group_number_open() {
		...
		updateText((float) 0.019);
		textOrder[0].setBackgroundColor(Color.rgb(0, 255, 0));
		textOrder[0].setText("体联IV——19#");
		String str = "十九";//但是不知道这两个字是怎么闪的，上机的时候观察一下
	...
	private void show_zu_open() {
		updateText((float) 0103);
		textOrder[6].setBackgroundColor(Color.rgb(0, 255, 0));
		textOrder[6].setText("全组点名已完成");
		String str = "组";
		FontClass.getInstance().setContent(str, this.getAssets());
		beepControl(2);
		updateClock(5);
	...
	private void gotoDeepDesign() {
		Intent intent = new Intent(this,
				com.example.du.deepPart.MainActivity.class);
