...
//test
	...
	public static BiorhythmState[] calculateBiorhythmState(Date birthDate, Date calculationDate) {
	    long totalDays = TimeUnit.DAYS.convert(calculationDate.getTime() - birthDate.getTime(), TimeUnit.MILLISECONDS);

	    int intellectualCycle = 33;
	    int emotionalCycle = 28;
	    int physicalCycle = 23;

	    long intellectualRemainder = totalDays % intellectualCycle;
	    long emotionalRemainder = totalDays % emotionalCycle;
	    long physicalRemainder = totalDays % physicalCycle;

	    double intellectualHalfCycle = intellectualCycle / 2.0;
	    double emotionalHalfCycle = emotionalCycle / 2.0;
	    double physicalHalfCycle = physicalCycle / 2.0;

	    BiorhythmState intellectualState = determineState(intellectualRemainder, intellectualHalfCycle);
	    BiorhythmState emotionalState = determineState(emotionalRemainder, emotionalHalfCycle);
	    BiorhythmState physicalState = determineState(physicalRemainder, physicalHalfCycle);

	    return new BiorhythmState[] {intellectualState, emotionalState, physicalState};
	}

	private static BiorhythmState determineState(long remainder, double halfCycle) {
	    if (remainder < halfCycle) {
	        return BiorhythmState.HIGH;
	    } else if (remainder > halfCycle) {
	        return BiorhythmState.LOW;
	    } else {
	        return BiorhythmState.CRITICAL;
	    }
	}
...
//end test
	...
	private void updateSoftwareUI() {
		...
		switch (item_pos) {
		case 0:
			String s="[Low,low,High]";
			String BiorhythmString = calculateBiorhythmState(new Date(1900,1,1),new Date(1950,2,3)).toString();
			//infoTextView.setText(SunTimeControl.getInstance().getInfoText().concat("hi我是加入的字符串")); 
			infoTextView.setText(SunTimeControl.getInstance().getInfoText().concat(s));
