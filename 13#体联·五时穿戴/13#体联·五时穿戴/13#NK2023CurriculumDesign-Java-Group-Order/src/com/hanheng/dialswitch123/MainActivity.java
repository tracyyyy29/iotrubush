/* *******************************************************
 * XX#NK2023CurriculumDesign-Java-Group-Order-template.c *
 * *******************************************************
 * 代码部分整体操作说明：									 *
 * 1.代码构成：											 *
 ...
 *  角色身份（如NK2023-01#-Le），具体位置的定	位以注释的形    *
 *  式在代码中给出从而实现定位功能。					     *
 *  -问题描述：											 *
 *    参见示例，阐释自己的问题，要求表述清晰，问题聚焦。          *
 *  -解决状态：											 *
 *    参见示例，分为未解决/已解决两部分。其中已解决应标明解   *
 *  决人的角色身份（如NK2023-01#-R）。                                                      *
 *  -解决策略：											 *
 ...
 *
 *
 *【问题定位（示例）】Q0(NK2023-XX#-Le)
 *【问题表述（示例）】updateText内部如何对数码管进行显示特定字符操作的？
 *【解决状态（示例）】已解决(NK2023-XX#-Lo)
...
public class MainActivity extends Activity implements OnClickListener{
	...
	//Author：XXX
	//Date：2022/XX/XX
	//Description：显示组号信息。组号"13"的汉字为"十三"，转换为4位四进制数为 0031Q
	public void show_group_number_open()
	{
																										//模板以数字"13"为例，"13"的汉字为"十三"，转换为4位四进制数为 0031Q。
																										//请按照本组真实组号对本函数进行修改
		/********请在下方区域修改代码及注释，实现利用点阵和数码管显示本组组号相关信息的功能********/
		
		updateText((float) 0031); 																			//数码管显示4位四进制数
		
		
		String str = "十三";  
		byte[][] data = FontClass.getInstance().setContent(str,this.getAssets()); 						//点阵显示汉字"十三"
	...
	//Author：XXX
	//Date：2022/XX/XX
	//Description：显示组长姓氏信息。组长姓氏的汉字为"王"，该姓氏的拼音首字母小写为"w"，其ASCII码为0111 0111B，转换成4位四进制数为 1313Q
	public void show_first_name_lea_open()
	{
																										//模板以"王"为例，"长"的拼音首字母小写为"w"，其ASCII码为 0111 0111B，转换成4位四进制数为1313Q
																										//请按照本组组长真实姓氏对本函数进行修改
	...
	//需求员姓氏的汉字为"魏"，该姓氏的拼音首字母小写为"w"，其ASCII码为 0111 0111B，转换成4位四进制数为 1313Q
